    ...
    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < 4; j++)
        {
            for (k = 0; k < 8; k++)
            {
                ...
                newsPrism.cell[i][j][k] = xInverseTable[(int)(sp ^ pl)] ^ xInverseTable[(int)(spp ^ ph)] ^ h;
...
HashReturn Update(hashState* state, const BitSequence* data, DataLength databitlen)
{
    ...
    if (state->remainderbitlen != 0)
    { /* handles any bits in state's remainder */
    ...
    memcpy(&state->remainder, &data[currentbyte],
           (state->remainderbitlen / 8) + (((state->remainderbitlen % 8) != 0) ? 1 : 0));
    return SUCCESS;
}
...
HashReturn Final(hashState* state, BitSequence* hashval)
{
    ...
    if (state->remainderbitlen != 0)
    { /* if there are remaining bits to be processed... */
        ...
        if (leftoverbits != 0)
        { /* if the remaining bits isn't a whole byte... */
...
{
    hashState state;
    if (Init(&state, hashbitlen) == BAD_HASHBITLEN)
    {/* initializes state and checks to make sure hashbitlen is valid */
        return BAD_HASHBITLEN;
    }
