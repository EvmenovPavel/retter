 ...
 */
int ReadChunk(buf)word32 buf[CHUNK_SIZE];
{
 ...
 */
void HashN(output, input)word32 output[OUTPUT_BLOCK_SIZE];
    ...
    for (index = 0; index < SECURITY_LEVEL; index++)
    {
        for (byteInWord = 0; byteInWord < 4; byteInWord++)
        {
            for (i = 0; i < INPUT_BLOCK_SIZE; i++)
            {
                ...
                SBoxEntry = standardSBoxes[2 * index + ((i / 2) & 1)][block[i] & 0xffL];
                /*
 ...
 * possible, it requires great caution.
 */
void Hash512(output, input)word32 output[OUTPUT_BLOCK_SIZE];
                           word32 input[INPUT_BLOCK_SIZE];
{
    static int shiftTable[4] = {16, 8, 16, 24};
    /* the array of data being hashed  */
 ...
 */
void Increment64BitCounter(counter, increment)word32 counter[2];
                                              long int increment;
{
