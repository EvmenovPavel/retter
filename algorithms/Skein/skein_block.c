...
{ /* do it in C */
    ...
    do
    {
        ...
        { /* unroll 8 rounds */
            ...
            X[3] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 7, X);
            ...
            X[1] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 6, X);
            ...
            X[3] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 5, X);
            ...
            X[1] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 4, X);
            InjectKey(2 * r - 1);
            ...
            X[3] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 3, X);
            ...
            X[1] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 2, X);
            ...
            X[3] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 1, X);
            ...
            X[1] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r, X);
            InjectKey(2 * r);
        }
...
{ /* do it in C */
    ...
    do
    {
        ...
        { /* unroll 8 rounds */
            ...
            X[7] ^= X[6];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 7, X);
            ...
            X[3] ^= X[0];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 6, X);
            ...
            X[7] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 5, X);
            ...
            X[3] ^= X[4];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 4, X);
            InjectKey(2 * r - 1);
            ...
            X[7] ^= X[6];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 3, X);
            ...
            X[3] ^= X[0];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 2, X);
            ...
            X[7] ^= X[2];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 1, X);
            ...
            X[3] ^= X[4];  Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r, X);
            InjectKey(2 * r);
        }
        /* do the final "feedforward" xor, update context chaining vars */
        for (i = 0; i < WCNT; i++)
...
{ /* do it in C */
    ...
    do
    {
        ...
        { /* unroll 8 rounds */
            ...
            X[15] ^= X[14];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 7, X);
            ...
            X[1] ^= X[8];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 6, X);
            ...
            X[9] ^= X[10];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 5, X);
            ...
            X[7] ^= X[12];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 4, X);
            InjectKey(2 * r - 1);
            ...
            X[15] ^= X[14];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 3, X);
            ...
            X[1] ^= X[8];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 2, X);
            ...
            X[9] ^= X[10];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r - 1, X);
            ...
            X[7] ^= X[12];    Skein_Show_Round(BLK_BITS, &ctx->h, 8 * r, X);
            InjectKey(2 * r);
        }
        /* do the final "feedforward" xor, update context chaining vars */
        for (i = 0; i < WCNT; i++)
