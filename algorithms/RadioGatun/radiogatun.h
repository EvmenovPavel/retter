...
void RadioGatun<W>::InverseRoundFunction()
{
    ...
    for (unsigned int i = 0; i < MillSize; i++)
        A[i] = a[(i + 3) % MillSize] ^ a[(i + 4) % MillSize] ^ a[(i + 5) % MillSize] ^ a[(i + 6) % MillSize] ^ a[(i + 8) % MillSize] ^ a[(i + 10) % MillSize] ^ a[(i + 11) % MillSize] ^ a[(i + 14) % MillSize] ^ a[(i + 18) % MillSize];
    // inverse of pi
    for (unsigned int i = 0; i < MillSize; i++)
        a[(7 * i) % MillSize] = ROR(A[i], -i * (i + 1) / 2);
    // inverse of gamma
    for (unsigned int i = 0; i < MillSize; i++)
        A[i] = ~a[i]; // complement input
    for (unsigned int i = 0; i < MillSize; i++)
        a[i] = A[i]; // inverse of Chi: sowing the seeds
    for (unsigned int j = 0; j < 3 * (MillSize - 1) / 2; j++)
    { // inverse of Chi: leaping 3*(MillSize-1)/2 times
