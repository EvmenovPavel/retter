...
void gen_keygen_matrix()
{
    ...
    for (z = 64; z >= 0; z--)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            key[i][0] = (w[0] & 0x000000ff) | ((w[2] & 0x000000ff) << 8) | ((w[4] & 0x000000ff) << 16) | ((w[6] & 0x000000ff) << 24);
            key[i][1] = ((w[0] & 0x0000ff00) >> 8) | (w[2] & 0x0000ff00) | ((w[4] & 0x0000ff00) << 8) | ((w[6] & 0x0000ff00) << 16);
            key[i][2] = ((w[0] & 0x00ff0000) >> 16) | ((w[2] & 0x00ff0000) >> 8) | (w[4] & 0x00ff0000) | ((w[6] & 0x00ff0000) << 8);
            key[i][3] = ((w[0] & 0xff000000) >> 24) | ((w[2] & 0xff000000) >> 16) | ((w[4] & 0xff000000) >> 8) | (w[6] & 0xff000000);
            key[i][4] = (w[1] & 0x000000ff) | ((w[3] & 0x000000ff) << 8) | ((w[5] & 0x000000ff) << 16) | ((w[7] & 0x000000ff) << 24);
            key[i][5] = ((w[1] & 0x0000ff00) >> 8) | (w[3] & 0x0000ff00) | ((w[5] & 0x0000ff00) << 8) | ((w[7] & 0x0000ff00) << 16);
            key[i][6] = ((w[1] & 0x00ff0000) >> 16) | ((w[3] & 0x00ff0000) >> 8) | (w[5] & 0x00ff0000) | ((w[7] & 0x00ff0000) << 8);
            key[i][7] = ((w[1] & 0xff000000) >> 24) | ((w[3] & 0xff000000) >> 16) | ((w[5] & 0xff000000) >> 8) | (w[7] & 0xff000000);
        ...
        for (i = 0; i < 4; i++)
        {
            for (j = 0; j < 32; j += 4)
            {
                ...
                keygen_matrix[i][j][z] = (l & 0xff ? 1 : 0) ^ keygen_matrix[i][j][64];
                keygen_matrix[i][j + 1][z] = ((l >> 8) & 0xff ? 1 : 0) ^ keygen_matrix[i][j + 1][64];
                keygen_matrix[i][j + 2][z] = ((l >> 16) & 0xff ? 1 : 0) ^ keygen_matrix[i][j + 2][64];
                keygen_matrix[i][j + 3][z] = ((l >> 24) & 0xff ? 1 : 0) ^ keygen_matrix[i][j + 3][64];
            }
        }
    }
}
