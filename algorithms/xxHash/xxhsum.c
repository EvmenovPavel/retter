    ...
    while (fileIdx < nbFiles)
    {
        ...
        // Bench XXH32
        {
            ...
            for (interationNb = 1; interationNb <= g_nbIterations; interationNb++)
            {
                ...
                if ((double)milliTime < fastestC * nbHashes)
                    fastestC = (double)milliTime / nbHashes;
                DISPLAY("%1i-%-14.14s : %10i -> %7.1f MB/s\r", interationNb, "XXH32", (int)benchedSize,
                        (double)benchedSize / fastestC / 1000.);
            }
            DISPLAY("%-16.16s : %10i -> %7.1f MB/s   0x%08X\n", "XXH32", (int)benchedSize,
                    (double)benchedSize / fastestC / 1000., hashResult);
        ...
        // Bench Unaligned XXH32
        {
            ...
            for (interationNb = 1; (interationNb <= g_nbIterations) && ((benchedSize > 1)); interationNb++)
            {
                ...
                if ((double)milliTime < fastestC * nbHashes)
                    fastestC = (double)milliTime / nbHashes;
                DISPLAY("%1i-%-14.14s : %10i -> %7.1f MB/s\r", interationNb, "XXH32 (unaligned)",
                        (int)(benchedSize - 1), (double)(benchedSize - 1) / fastestC / 1000.);
            }
            DISPLAY("%-16.16s : %10i -> %7.1f MB/s \n", "XXH32 (unaligned)", (int)benchedSize - 1,
                    (double)(benchedSize - 1) / fastestC / 1000.);
        }
        ...
        // Bench XXH64
        {
            ...
            for (interationNb = 1; interationNb <= g_nbIterations; interationNb++)
            {
                ...
                if ((double)milliTime < fastestC * nbHashes)
                    fastestC = (double)milliTime / nbHashes;
                DISPLAY("%1i-%-14.14s : %10i -> %7.1f MB/s\r", interationNb, "XXH64", (int)benchedSize,
                        (double)benchedSize / fastestC / 1000.);
            }
            DISPLAY("%-16.16s : %10i -> %7.1f MB/s   0x%08X%08X\n", "XXH64", (int)benchedSize,
                    (double)benchedSize / fastestC / 1000., (U32)(h64 >> 32), (U32)(h64));
...
static void BMK_checkResult(U32 r1, U32 r2)
{
    ...
    if (r1 == r2)
        DISPLAY("\rTest%3i : %08X == %08X   ok   ", nbTests, r1, r2);
    else
    {
...
static void BMK_testSequence64(void* sentence, int len, U64 seed, U64 Nresult)
{
    ...
    for (index = 0; index < len; index++)
        XXH64_update(&state, ((char*)sentence) + index, 1);
    Dresult = XXH64_digest(&state);
    BMK_checkResult64(Dresult, Nresult);
}
...
static void BMK_testSequence(void* sentence, int len, U32 seed, U32 Nresult)
{
    ...
    XXH32_reset(&state, seed);
    for (index = 0; index < len; index++)
        XXH32_update(&state, ((char*)sentence) + index, 1);
    Dresult = XXH32_digest(&state);
    BMK_checkResult(Dresult, Nresult);
}
...
int main(int argc, char** argv)
{
    ...
    if (strstr(exename, "xxh32sum") != NULL)
        g_fn_selection = 0;
    ...
    if ((input_filename == stdinName) && IS_CONSOLE(stdin))
        return badusage(exename);
    ...
    if (g_fn_selection < 0 || g_fn_selection > 1)
        return badusage(exename);
